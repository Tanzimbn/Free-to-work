...
exports.post_filter = async (req, res) => {
    ...
    for(let i = 0; i < post.length; i++) {
        ...
        if(req.body.searchValue != "" && !post[i].title.toLowerCase().includes(req.body.searchValue.toLowerCase()) && !post[i].category.toLowerCase().includes(req.body.searchValue.toLowerCase())) {
            continue;
        }
